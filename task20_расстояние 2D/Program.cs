...
int squareX = Math.Pow(userBx - userAx, 2);// Math.Pow(a, 3) функция 
возведения в степень,(что возводим в степень, в какую степень)

System.Console.WriteLine(Math.Round(rootXY, 2));//Math.Round(a, 2) округление чисел после 
запятой(число, сколько чисел после запятой оставить)

double rootXY = Math.Sqrt(squareX + squareY);//Math.Sqrt() 
...
SConsole.WriteLine("Ведите координаты точки B (x, y): ");
